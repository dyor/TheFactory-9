Write or update an AGENTS.md file that includes the following agent instructions. 

# Project Overview: Film Factory

## Introduction
This is a **Kotlin Multiplatform (KMP)** project called "Film Factory". It targets **Android**, **iOS**, and **Desktop (JVM)**. It uses **Compose Multiplatform** for sharing UI code across these platforms.
...
3.  **Desktop (`:desktopApp`)**
    *   Kotlin/JVM application for Windows/macOS/Linux.
    *   Runs the shared UI in a desktop window.
...
2.  **Platform Divergence:** If you need platform-specific behavior (e.g., Camera, Permissions, SQL Drivers), use the `expect` keyword in `commonMain` and implement `actual` in `androidMain`, `iosMain`, `jvmMain` etc.
